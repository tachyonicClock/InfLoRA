...
class ViT_Prompts(VisionTransformer):
    ...
    def forward(
        self, x, register_blk=-1, prompt=None, q=None, train=None, task_id=None
    ):
...
class SiNet(nn.Module):
    def __init__(self, args):
        ...
        self.prompt_pool = CodaPrompt(
            args["embd_dim"], args["total_sessions"], args["prompt_param"]
        )
    ...
    def forward(self, image, feature):
        ...
        image_features, prompt_loss = self.image_encoder(
            ...
            train=True,
            task_id=(self.numtask - 1) * torch.ones(len(image)).to(image.device).long(),
        )
        image_features = image_features[:, 0, :]
    ...
    def interface(self, image, feature):
        image_features, _ = self.image_encoder(
            image, q=feature, prompt=self.prompt_pool, train=True
        )
        image_features = image_features[:, 0, :]
        image_features = image_features.view(image_features.size(0), -1)
    ...
    def update_fc(self, nb_classes):
        self.numtask += 1
        if self.numtask > 1:
            self.prompt_pool.process_task_count()
