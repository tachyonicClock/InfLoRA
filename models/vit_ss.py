...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        num_g_prompt = (
            len(self.g_prompt_layer_idx) if self.g_prompt_layer_idx is not None else 0
        )
        ...
        if (
            use_g_prompt
            and g_prompt_length is not None
            and len(g_prompt_layer_idx) != 0
        ):
        ...
        if self.prompt_pool:
            ...
            if not self.use_prefix_tune_for_e_prompt:
                self.total_prompt_len += (
                    prompt_length * top_k * len(self.e_prompt_layer_idx)
                )
        ...
        if mlp_structure:
            self.mlp = MlpMapping(
                dim=embed_dim, hidden=mlp_structure, fc_with_norm=use_fc_norm
            )
        else:
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        if self.use_prefix_tune_for_g_prompt:
                            ...
                            idx = torch.tensor([g_prompt_counter] * x.shape[0]).to(
                                x.device
                            )
...
def vit_base_patch16_224_mocov3(pretrained=False, **kwargs):
    ...
    # del model.head
    ckpt = torch.load("./checkpoints/mocov3-vit-base-300ep.pth", map_location="cpu")[
        "model"
    ]
    state_dict = model.state_dict()
    not_in_k = [k for k in ckpt.keys() if k not in state_dict.keys()]
...
def vit_base_patch16_224_mae(pretrained=False, adapter=False, **kwargs):
    ...
    ckpt = torch.load("./checkpoints/mae_pretrain_vit_base.pth", map_location="cpu")[
        "model"
    ]
    state_dict = model.state_dict()
    state_dict.update(ckpt)
...
def vit_small_patch16_224_ims(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_vision_transformer(
        "vit_small_patch16_224_in21k", pretrained=False, **model_kwargs
    )
    # del model.head
    state_dict = model.state_dict()
