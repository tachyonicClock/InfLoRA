...
class Attention_LoRA(nn.Module):
    def __init__(
        ...
        self.lora_A_k = nn.ModuleList(
            [nn.Linear(dim, r, bias=False) for _ in range(n_tasks)]
        )
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        if get_feat:
            self.matrix = (
                ...
                + torch.bmm(x.detach().permute(0, 2, 1), x.detach()).sum(dim=0).cpu()
            ) / (self.n_matrix + x.shape[0] * x.shape[1])
            self.n_matrix += x.shape[0] * x.shape[1]
        if get_cur_feat:
            self.cur_matrix = (
                self.cur_matrix * self.n_cur_matrix
                + torch.bmm(x.detach().permute(0, 2, 1), x.detach()).sum(dim=0).cpu()
        ...
        weight_k = torch.stack(
            [
                torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight)
        ...
        weight_v = torch.stack(
            [
                torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight)
                for t in range(task + 1)
            ],
            dim=0,
        ).sum(dim=0)
        k = k + F.linear(x, weight_k).reshape(
            B, N, self.num_heads, C // self.num_heads
        ).permute(0, 2, 1, 3)
        v = v + F.linear(x, weight_v).reshape(
            B, N, self.num_heads, C // self.num_heads
        ).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        x = x + self.drop_path1(
            self.ls1(self.attn(self.norm1(x), register_hook=register_hook))
        )
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed_grow = nn.Parameter(
            torch.zeros(1, num_patches + 1000, embed_dim)
        )
    ...
    def init_weights(self, mode=""):
        ...
        trunc_normal_(self.pos_embed_grow, std=0.02)
    ...
    def forward_features_grow(self, x, class_num):
        ...
        x = self.pos_drop(x + self.pos_embed)
        x = torch.cat(
            (self.cls_token_grow[:, : class_num * 2, :].expand(x.shape[0], -1, -1), x),
            dim=1,
        )
