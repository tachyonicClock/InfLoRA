...
class Attention_LoRA(nn.MultiheadAttention):
    def __init__(self, dim, num_heads=8, r=1, c=0.3):
        super().__init__(embed_dim=dim, num_heads=num_heads)
        self.total_sessions = 10
        self.lora_A_k = nn.ModuleList([nn.Linear(dim, r, bias=False)])
        self.lora_B_k = nn.ModuleList([nn.Linear(r, dim, bias=False)])
    ...
    def forward(
        ...
        if task > -0.5:
            ...
            mask_A_k = torch.zeros_like(self.lora_sA_k[task].weight)
            mask_B_k = torch.zeros_like(self.lora_sB_k[task].weight)
            mask_A_v = torch.zeros_like(self.lora_sA_v[task].weight)
            mask_B_v = torch.zeros_like(self.lora_sB_v[task].weight)
    ...
    def rectify_grad(self, task):
        with torch.no_grad():
            ...
            cmask_B_k = torch.zeros_like(self.lora_sB_k[0].weight)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, task, instance_tokens=None):
